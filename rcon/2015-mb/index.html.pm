...
◊main-menu-title{(fifth RacketCon)}
◊(define rc-2015-date "27 Sept 2015")
◊main-menu["venue"]{◊rc-2015-date}
◊main-menu["venue"]{St. Louis}
◊main-menu{Speakers}
◊main-menu{Register}

◊(codebox (opacity-control (insert-file "eero-demo.rkt")))
...
◊(require [prefix-in recs: "data/speakers.pm"])
◊(define records (cdr recs:doc))
◊(require xml/path)

◊h3{◊xtarget["speakers"]{Keynote speaker}}
◊folded{
◊keynote-speaker["" ◊(car (se-path*/list '(name) (car records)))]{◊(car (se-path*/list '(title) (car records)))}

◊bar(se-path*/list '(desc) (car records))

◊foo(se-path*/list '(bio) (car records))}
