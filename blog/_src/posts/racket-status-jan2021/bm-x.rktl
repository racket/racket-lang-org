...
(define old-racket-color color:old-racket-color)
...
(define (shootout-benchmarks #:t t #:bt bt #:tt tt #:gap-size gap-size #:bar-sep [bar-sep 0]
  ...
  (rb-superimpose
   (plot #:t t #:tt tt #:gap-size gap-size #:bar-sep bar-sep #:bar-t bt
         #:bar-text-scale bar-text-scale
         #:columns 6
         #:sort-ratio (if prev-cs '(0 . 2) '(0 . 1))
         #:prefix (if mean? "x" "")
         #:suffix (if mean? "" " msec")
         #:decimal-places (if mean? 2 0)
         #:key->pict (lambda (k) label)
         #:width width
         (append (if prev-cs (list (t old-RacketCS-abbrev)) null)
                 (list (t RacketCS-abbrev) (t Racket-abbrev)))
         (append (if prev-cs (list old-racket-color) null)
                 (list racket-color c-color))
         (if mean?
             (list (geometric-mean shootout-r/cs shootout-r)
                   (geometric-mean shootout-r shootout-r))
             (append (if prev-cs (list prev-cs) null)
...
(module+ pict
  ...
  (define (shootout-benchmarks-table #:prev-cs [prev-cs #f])
    (scale (shootout-benchmarks #:t t #:bt bt #:tt tt #:gap-size gap-size #:mean? #f
                                #:prev-cs prev-cs)
           0.20)))
