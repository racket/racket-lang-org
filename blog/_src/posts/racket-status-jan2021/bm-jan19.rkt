...
(define RacketCS-abbrev "R/CS")
(define old-RacketCS-abbrev "old CS")
...
(module+ pict
  ...
  (define (one-plot label-str
                    ...
                    #:old-CS [old-RacketCS-abbrev old-RacketCS-abbrev]
                    #:CS [RacketCS-abbrev RacketCS-abbrev]
                    #:R [Racket-abbrev Racket-abbrev])
    (define key (string->symbol label-str))
    (define (mk n) (if n (list (hash key n)) null))
    (define names
      (append
       (if old-cs (list (t old-RacketCS-abbrev)) null)
    ...
    (scale
     (add-desc
      ...
      (plot #:t t #:tt tt #:gap-size gap-size #:bar-sep bar-sep #:bar-t bar-t
            ...
            names
            (append
             (if old-cs (list old-racket-color) null)
            ...
            (append
             (if old-cs (list (hash key old-cs)) null)
             (list (hash key cs)
                   (hash key r))
             (mk r-cify)
             (mk r6)
